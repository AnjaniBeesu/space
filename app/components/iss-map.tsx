...
export default function IssMap({ position, trail }: { position: Position; trail: Position[] }) {
  return (
    <MapContainer center={[position.latitude, position.longitude]} zoom={2} minZoom={2} maxZoom={6} scrollWheelZoom className="iss-map">
      ...
      />
      {trail.length > 1 && <Polyline positions={trail.map((p) => [p.latitude, p.longitude] as [number, number])} pathOptions={{ color: "#ff6bd6", weight: 3, opacity: 0.7 }} />}
      ...
      <CircleMarker center={[position.latitude, position.longitude]} radius={18} pathOptions={{ color: "#ff6bd6", weight: 1, fillOpacity: 0, opacity: 0.35 }} />
